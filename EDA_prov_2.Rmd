...
data4 <- data3 %>% dplyr::mutate(N_totales = N_rcmat + N_rccorp) %>%
                  dplyr::select(-poliza,-Edad,-Carne,-fini,-fvto,-Veh_cdin,-Veh_val,-Veh_peso,-N_rcmat,-N_rccorp,-N_culpa,-N_inoc,-C_rcmat_agregado,-C_rccorp,-fpag,-Edad_Cat,-veh_ant,-Cus_des,-clase_veh,-Expo)
...
summary(NB.log)
# quitamos la marca porque no es significativa
NB.log <- update(NB.log, ~.-Veh_marca)
summary(NB.log)

# quitamos el valor del vehiculo porque no es significativa
NB.log <- update(NB.log, ~.-Veh_val_Cat)
summary(NB.log)

# quitamos el tipo de vehiculo porque no es significativa
NB.log <- update(NB.log, ~.-Tipo_veh)
summary(NB.log)

# quitamos el numero de puertas del vehiculo porque no es significativa
NB.log <- update(NB.log, ~.-b7_puertas)
summary(NB.log)
```

```{r}
# hacemos un step con lo que tenemos a ver qué modelo nos propone 
...
```{r}
# hacemos un summary para ver el resultado del último modelo 
...
Probamos a hacer primero el step y luego eliminar variables que no sean significativas.
```{r}
NB.log2 <- glm.nb(N_totales~ . ,data=train_data,link="log",control=glm.control(maxit=150))
selec2 <- step(NB.log2, direction='both', trace=1)
```

```{r}
summary(selec2)

selec2 <- update(selec2, ~. -Veh_marca)
summary(selec2)

selec2 <- update(selec2, ~. -b7_puertas)
summary(selec2)

selec2 <- update(selec2, ~. -Tipo_veh)
summary(selec2)

selec2 <- update(selec2, ~. -Veh_val_Cat)
summary(selec2)
```
