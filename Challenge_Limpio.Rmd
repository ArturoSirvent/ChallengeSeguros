...
data4 <- data3 %>% dplyr::mutate(N_totales = N_rcmat + N_rccorp) %>%
                  dplyr::select(-poliza,
                              ...
                              -fini,-fvto,
                              -Veh_cdin,-Veh_val,
                              -Veh_peso,-Veh_cuso,-N_rcmat,-N_rccorp,-N_culpa,-N_inoc,-C_rccorp,
                              -Edad_Cat,
...
```{r}
plot(p.NB.log, test_data$N_totales)
```
...
rf <- randomForest(factor(N_totales) ~ .-Veh_marca,data=train_data, proximity=TRUE,ntree=200)
print(rf)
```

```{r}
p1 <- predict(rf, test_data)
confusionMatrix(p1, factor(test_data$N_totales))
```
...
write.csv(resultados, file='./resultados_1mayo.csv', row.names = F)
```
